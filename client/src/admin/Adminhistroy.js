        ...
        const fetchData = async () => {
            try {
                ...
                const filteredFormData = response.data.data.filter(form => {		
					return (form.tripStatus === 1 && form.status !== 0) || form.status === 0 
							...
							<>
								...
								{formData.map(trip => (
									<div key={trip._id} className="col-md-4 mb-4">
										<div className="card shadow" style={{ border: 'none',height:'550px' }}>
											<div className="card-body">
												<div className="row">
													...
													<div className="col-md-6">
														...
														<img src={trip.signature} className="card-img-top mb-3 " alt="UserPhoto" style={{height:'80px'}}/>
														<p className="card-text"><strong style={{ color: '#17a2b8' }}>Trip Status:</strong> { trip.status === 0 && <span style={{ color: 'green' }}>Rescued Successfully by {trip.alertData} Police Station </span>}
														{trip.tripStatus === 1 && trip.status !== 0 && <span style={{ color: 'green' }}>Reached Destination</span>} </p>
